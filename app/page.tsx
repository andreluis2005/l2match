...
export default function QuizPage() {
  ...
  const handleAnswer = async (answer: string, index: number) => {
    ...
    if (currentQuestion + 1 < questions.length) {
      ...
      const newResult = topL2s.map(([l2]) => 
        l2 === 'Arbitrum' ? 'Arbitrum: Technical Explorer!' :
        l2 === 'Optimism' ? 'Optimism: Social Idealist!' :
        l2 === 'Base' ? 'Base: Practical Innovator!' :
  ...
  const resetQuiz = () => {
    ...
    setResult(null);
    setErrorMessage(null); // Limpa mensagem de erro ao reiniciar
  };
  ...
  const shareOnWarpcast = () => {
    if (result) {
      // Extrai o nome do L2 do resultado (ex.: "Base" de "Base: Practical Innovator!")
      const l2Name = result.split(' & ')[0].split(':')[0].toLowerCase();
      // Gera o link dinâmico para o resultado
      const scoreGraphLink = `https://l2match.vercel.app/result?score=${l2Name}`;
      // Monta a mensagem de compartilhamento com o link substituído
      ...
      // Abre o Warpcast com a mensagem
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:description" content="Take this fun quiz to find out which Layer 2 blockchain matches your personality!" />
      ...
      {(!address && connectors.length > 0) && (
        <button
          className={`px-4 py-2 rounded-lg font-semibold ${isDarkMode ? 'bg-purple-500 text-gray-100 hover:bg-purple-400' : 'bg-indigo-600 text-white hover:bg-indigo-700'} transition-all duration-200 shadow-md mb-4`}
      ...
      <div className={`min-h-screen flex flex-col items-center justify-between p-6 ${isDarkMode ? 'bg-gray-950 text-gray-100' : 'bg-gradient-to-br from-indigo-100 via-purple-50 to-pink-100 text-gray-800'}`}>
        <header className="w-full max-w-[520px] flex justify-between items-center py-4">
          <h1 className={`text-2xl font-bold ${isDarkMode ? 'text-purple-400 drop-shadow-md' : 'text-indigo-800 drop-shadow-md'}`}>Which L2 Are You?</h1>
          <div className="flex space-x-3">
            <button
              className={`bg-${isDarkMode ? 'purple-500' : 'indigo-600'} text-white px-4 py-2 rounded-lg shadow-md hover:bg-${isDarkMode ? 'purple-400' : 'indigo-700'} hover:shadow-lg transition-all duration-200`}
              onClick={handleAddFrame}
            >
              Save App
            </button>
            <button
              className={`bg-transparent ${isDarkMode ? 'text-purple-400' : 'text-indigo-800'} font-semibold text-sm hover:underline`}
              onClick={handleClose}
            >
              Close
            </button>
            <button
              className={`px-3 py-1 rounded-full text-sm font-semibold ${isDarkMode ? 'bg-gray-800 text-purple-400 hover:bg-gray-700' : 'bg-white text-indigo-600 border border-indigo-300 hover:bg-gray-100'}`}
              onClick={toggleTheme}
            >
        ...
        <main className="flex-1 flex flex-col items-center justify-center w-full max-w-[520px]">
          <div className={`w-full ${isDarkMode ? 'bg-gray-800' : 'bg-gray-200'} rounded-full h-2.5 mb-4`}>
            <div
              className={`h-2.5 rounded-full transition-all duration-300 ${isDarkMode ? 'bg-purple-500' : 'bg-indigo-600'}`}
              style={{ width: `${progress}%` }}
            ></div>
          </div>

          {quizFinished ? (
            <div className={`text-center ${isDarkMode ? 'bg-gray-800' : 'bg-white'} p-6 rounded-lg shadow-lg`}>
              <h2 className={`text-3xl font-bold ${isDarkMode ? 'text-purple-400' : 'text-indigo-800'} mb-4`}>Your L2 Soulmate!</h2>
              <p className={`text-xl ${isDarkMode ? 'text-gray-100' : 'text-gray-700'} mb-4`}>{result || 'Error loading result'}</p>
              <div className="mb-6">
                <h3 className={`text-lg font-semibold ${isDarkMode ? 'text-purple-400' : 'text-indigo-800'} mb-2`}>Score Distribution</h3>
                <div className="space-y-3">
                  {Object.entries(scorePercentages).map(([l2, percentage]) => (
                    <div key={l2} className="flex items-center">
                      <span className={`w-24 text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>{l2}</span>
                      <div className={`flex-1 h-4 ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'} rounded-full overflow-hidden`}>
                        <div
                          className={`h-full ${l2Colors[l2 as keyof typeof l2Colors]} rounded-full transition-all duration-500`}
                          style={{ width: `${percentage}%` }}
                        ></div>
                      </div>
                      <span className={`ml-3 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{score[l2 as keyof typeof score]}/{maxPossibleScore}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div className="flex flex-col items-center">
                <button
                  className={`px-4 py-2 rounded-lg font-semibold ${isDarkMode ? 'bg-purple-500 text-gray-100 hover:bg-purple-400' : 'bg-indigo-600 text-white hover:bg-indigo-700'} transition-all duration-200 shadow-md mb-2`}
                ...
                <button
                  className={`px-4 py-2 rounded-lg font-semibold ${isDarkMode ? 'bg-purple-500 text-gray-100 hover:bg-purple-400' : 'bg-indigo-600 text-white hover:bg-indigo-700'} transition-all duration-200 shadow-md`}
                  onClick={handleSaveResult}
                  disabled={isLoading || !address}
                >
                  {isLoading ? 'Saving...' : 'Save Result Onchain'}
                </button>
                {(!address || errorMessage) && <p style={{ color: 'red' }}>{errorMessage || 'Wallet not connected. Connect to MetaMask.'}</p>}
              </div>
            </div>
          ) : (
            <div className={`w-full ${isDarkMode ? 'bg-gray-800' : 'bg-white'} p-6 rounded-lg shadow-lg`}>
              <h2 className={`text-2xl font-semibold ${isDarkMode ? 'text-purple-400' : 'text-indigo-800'} mb-6`}>
              ...
              <div className="grid grid-cols-1 gap-4">
                {questions[currentQuestion].options.map((option) => (
                  <button
                    key={option}
                    className={`bg-${isDarkMode ? 'gray-800' : 'white'} border-2 ${isDarkMode ? 'border-purple-500' : 'border-indigo-300'} rounded-lg p-4 text-center ${isDarkMode ? 'text-gray-100' : 'text-gray-800'} font-medium hover:bg-${isDarkMode ? 'gray-700' : 'indigo-100'} hover:border-${isDarkMode ? 'purple-400' : 'indigo-400'} transition-all duration-200 transform hover:scale-105 shadow-sm`}
        ...
        <footer className="w-full max-w-[520px] flex justify-center py-4">
          <button
            className={`px-4 py-2 rounded-full font-semibold ${isDarkMode ? 'text-gray-400 bg-gray-800 border-gray-600 hover:bg-gray-700' : 'text-gray-600 bg-white border border-gray-300 hover:bg-gray-100'} text-sm transition-all duration-200 shadow-sm`}
